			...
			foreach ($_GET as $key => $value) {
				...
				if($num == 1){
					if($key == "page")
					{
						...
						$next_page_link  .= "page" . "=" . ($page_number + 1);
...
function i_own_content($row){
	...
	$myid = $_SESSION['socialsite_userid'];
	//profile
	...
	if(isset($row['postid'])){
		...
		{
			$Post = new Post();
			$one_post = $Post->get_one_posts($row['parent']);
...

{
	...
	foreach ($tags as $tag) {
		...
		add_notification($_SESSION['socialsite_userid'],"tag",$mypost,$tagged_user['userid']);
		}
	}else{
...
if(count($follow)>0){
	$str = "'" .  implode("','",$follow) . "'";
	$query = "select * from notifications where (userid != '$userid' && content_owner = '$userid') || (userid != '$userid' && contentid in ($str)) order by id desc limit 30";
}
else
{
...
if(isset($_SESSION['socialsite_userid'])){
	...
	set_online($_SESSION['socialsite_userid']);
}
...
function check_messages()
	{
			...
			$query = "select * from messages where (receiver = '$me' && seen = 0) limit 100";

...

		{
			...
			if(isset($data['parent']) && is_numeric($data['parent'])){
				...
				if(is_array($mypost) && $mypost['userid'] != $userid){
					...
					//add notification
					add_notification($_SESSION['socialsite_userid'],"comment",$mypost);
				}
			...
			$query = "insert into posts (userid,postid,post,image,has_image,is_cover_image,is_profile_image,parent,tags) values ('$userid','$postid','$post','$myimage','$has_image','$is_cover_image','$is_profile_image','$parent','$tags')";
...
public function edit_post($data, $files)
	{
...

		{
			...
			if($has_image)
			{
			$query = "update posts set post = '$posts' , image = '$myimage' where postid = '$postid' limit 1";
		}else
		{
	...
	public function delete_posts($postid)
	{
		...
		$Post = new Post();
		$one_post = $Post->get_one_posts($postid);
		$DB = new Database();
...
//delet any image and thumbnail
	...
	public function i_own_post($postid,$socialsite_userid )
		{
			...
			$query = "select * from posts where postid = '$postid' limit 1";
				...
				if(is_array($result))
				{
					...
					if($result[0]['userid'] == $socialsite_userid)
					{
						return true;
		...
		public function like_post($id,$type,$socialsite_userid){
				$DB = new Database();
			...
			if(is_array($result))
			{
				...
				if(!in_array($socialsite_userid, $user_ids)){


				$arr["userid"] = $socialsite_userid;
			...
			$single_post = $post->get_one_posts($id);
						//add notification
			add_notification($_SESSION['socialsite_userid'],"like",$single_post);
		}
			}else
			{


				$key = array_search($socialsite_userid,$user_ids);
				unset($likes[$key]);

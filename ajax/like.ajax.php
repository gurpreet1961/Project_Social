...
//check if not login
	if($_SESSION['socialsite_userid'] == 0)
	{
			$obj = (object)[];
	...
	if(isset($_GET['type']) && isset($_GET['id'])){
		...
		if(is_numeric($_GET['id'])){
			...
			if(in_array($_GET['type'] , $allowed))
			{
				...
				$post->like_post($_GET['id'],$_GET['type'],$_SESSION['socialsite_userid']);
				
				if($_GET['type'] == "user"){
					$user_class->follow_user($_GET['id'],$_GET['type'],$_SESSION['socialsite_userid']);
			}

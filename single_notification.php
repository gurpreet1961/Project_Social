<?php
	...
	//check if the notification is seen
...
<div id ="notification" style="background-color: <?=$color?>">
	...
	echo "<img src='$image' style='width:36px;margin:4px;float:left; margin-right:10px;' />";
if($actor['userid'] != $id){
	...
	$content_row = $Post->get_one_posts($notif_row['contentid']);
	if($notif_row['content_type'] == "post"){
		...
		if(file_exists($content_row['image']))
			{
				...
				echo "<img src='$post_image' style = 'width: 45px; float:right;margin:2px;' />";
			}
		}else{
		...
		echo $notif_row['content_type'];
		if(isset($content_row['post'])){
		echo "<span style='float:right;font-size:11px;color:#888;display:inline-block;margin-right:10px;'>'".htmlspecialchars(substr($content_row['post'],0,50))."'</span>";}
